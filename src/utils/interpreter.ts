...
import { sendChatRequest, LLMTruncatedError } from './llm-request';
...
export async function sendToLLM(promptContext: string, content: string, promptVariables: PromptVariable[], model: ModelConfig): Promise<{ promptResponses: any[] }> {
	...
	try {
		const llmResponseContent = await sendChatRequest(provider, model, {
			system: systemContent,
			context: promptContext,
			payload: JSON.stringify(promptContent)
		});
		...
		// A truncated response still consumed the request — keep the cooldown armed
